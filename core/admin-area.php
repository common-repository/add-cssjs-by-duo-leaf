...
class dl_acj_AdminArea {
    ...
    public function adminEnqueueScripts() {
        ...
        wp_register_style('dl_acj_css_custom', WP_PLUGIN_URL . '/' . $this->pluginInfo->name . '/assets/css/custom.css', array(), null, 'all');
        wp_register_style('dl_acj_css_bootstrap', WP_PLUGIN_URL . '/' . $this->pluginInfo->name . '/assets/css/bootstrap-iso.css', array(), null, 'all');
        wp_register_style('dl_acj_css_bootstrap_theme', WP_PLUGIN_URL . '/' . $this->pluginInfo->name . '/assets/css/bootstrap-theme.css', array(), null, 'all');
        wp_register_style('dl_acj_css_bootstrap_toggle', WP_PLUGIN_URL . '/' . $this->pluginInfo->name . '/assets/css/bootstrap-toggle.min.css', array(), null, 'all');
    }

    public function adminRegisterScripts() {

        if (is_admin() && isset($this->get['page']) && $this->get['page'] === $this->pluginInfo->name) {

            wp_enqueue_script('dl_acj_customJS');
            wp_enqueue_script('dl_acj_bootstrap');
            wp_enqueue_script('dl_acj_bootstrapToggle');

            wp_enqueue_style('dl_acj_css_custom');
            wp_enqueue_style('dl_acj_css_bootstrap');
            wp_enqueue_style('dl_acj_css_bootstrap_theme');
            wp_enqueue_style('dl_acj_css_bootstrap_toggle');
        }
    }
